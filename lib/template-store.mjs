...
const layoutToXlsx = (bytes, record) => {
  ...
  const maxColumns = Math.max(1, ...(layout.document.pages || []).flatMap(page => (page.rows || []).map(row => (row.columns || []).reduce((sum, column) => sum + columnNeed(column), 0)))); const columnWidths = Array(maxColumns).fill(10);
  ...
  const addCell = (row, column, value, style) => ensureRow(row).cells.set(column, { value: valueText(value).replace(/\r\n/g, '\n'), style });
  ...
  const renderParagraphs = (paragraphs, startRow, startColumn, span) => { let row = startRow; for (const paragraph of paragraphs || []) { const value = paragraphText(paragraph, null); addCell(row, startColumn, value, paragraphStyle(paragraph)); addMerge(row, startColumn, row, startColumn + span - 1); const size = Math.max(defaultFontSize, ...(paragraph.children || []).map(child => parsePt(child.fontSize))); ensureRow(row).height = Math.max(15, size * 1.5 * Math.max(1, value.split('\n').length)); row++; } return row; };
  const renderTable = (table, startRow, startColumn, span, regionWidth) => {
    ...
    for (let index = 0; index < Math.min(span, tableColumns); index++) columnWidths[startColumn + index] = Math.max(columnWidths[startColumn + index], regionWidth * weights[index] / totalWeight);
    for (const [targetOffset, entry] of entries.entries()) { const rowIndex = startRow + targetOffset; for (const [column, cell] of (entry.templateRow.cells || []).entries()) { if (column >= span) break; const covering = (table.merges || []).find(merge => entry.source >= merge.rowIndex && entry.source < merge.rowIndex + merge.rowSpan && column >= merge.colIndex && column < merge.colIndex + merge.colSpan); if (covering && (entry.source !== covering.rowIndex || column !== covering.colIndex)) continue; const paragraphs = cell.content || []; const value = paragraphs.map(paragraph => paragraphText(paragraph, entry.context)).join('\n'); addCell(rowIndex, startColumn + column, value, paragraphStyle(paragraphs[0], cell, true)); const size = Math.max(defaultFontSize, ...paragraphs.flatMap(paragraph => (paragraph.children || []).map(child => parsePt(child.fontSize)))); ensureRow(rowIndex).height = Math.max(18, size * 1.5 * Math.max(1, value.split('\n').length)); } }
  ...
  for (const [pageIndex, page] of (layout.document.pages || []).entries()) { if (pageIndex && rows.length) pageBreaks.push(rows.length); for (const pageRow of page.rows || []) { const columns = pageRow.columns || []; const needs = columns.map(columnNeed); const totalWidth = columns.reduce((sum, column) => sum + Number(column.width || 100 / Math.max(1, columns.length)), 0) || 100; let startColumn = 0; const baseRow = rows.length; let rowEnd = baseRow; for (const [columnIndex, column] of columns.entries()) { const span = needs[columnIndex]; const regionWidth = printableWidth * Number(column.width || 100 / Math.max(1, columns.length)) / totalWidth; for (let offset = 0; offset < span; offset++) columnWidths[startColumn + offset] = Math.max(columnWidths[startColumn + offset], regionWidth / span); let cursor = baseRow; for (const block of column.blocks || []) { if (Number(block.type) === 4) cursor = renderTable(block.table || {}, cursor, startColumn, span, regionWidth); else cursor = renderParagraphs(block.content, cursor, startColumn, span); } rowEnd = Math.max(rowEnd, cursor); startColumn += span; } while (rows.length < rowEnd) ensureRow(rows.length); } }
  if (!rows.length) ensureRow(0);
  const cell = (entry, column, row) => `<c r="${excelColumnName(column)}${row}"${entry.style ? ` s="${entry.style}"` : ''} t="inlineStr"><is><t xml:space="preserve">${xmlEscape(entry.value)}</t></is></c>`;
