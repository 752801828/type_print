...
const parseXlsxStyles = zip => {
  const xml = zip.file('xl/styles.xml')?.asText() || ''; const section = name => xml.match(new RegExp(`<${name}\\b[^>]*>([\\s\\S]*?)<\\/${name}>`))?.[1] || '';
...
const xlsxPreviewHtml = bytes => {
  ...
  const renderSheet = sheet => { const xml = zip.file(sheet.path)?.asText() || ''; const cells = new Map(); const rowHeights = new Map(); const hiddenRows = new Set(); let maxRow = 0; let maxCol = 0;
