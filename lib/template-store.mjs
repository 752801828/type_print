...
const wordRun = (node, scope, context) => {
  const value = node?.type === 'variable' ? layoutValue(node.name, scope, context) : node?.text || '';
  const size = Math.max(9, parsePt(node?.fontSize || 10));
  const properties = `<w:rFonts w:ascii="Microsoft YaHei" w:hAnsi="Microsoft YaHei" w:eastAsia="Microsoft YaHei"/><w:sz w:val="${Math.round(size * 2)}"/><w:szCs w:val="${Math.round(size * 2)}"/>${node?.bold ? '<w:b/>' : ''}${node?.italic ? '<w:i/>' : ''}`;
  return `<w:r><w:rPr>${properties}</w:rPr>${wordText(value)}</w:r>`;
};
const wordParagraph = (paragraph, scope, context) => {
  const align = ['left', 'right', 'center', 'justify'].includes(paragraph?.align) ? paragraph.align : 'left';
  const size = Math.max(9, parsePt(paragraph?.children?.find(child => child?.fontSize)?.fontSize || 10));
  ...
  const runs = (paragraph?.children || []).map(node => wordRun(node, scope, context)).join('');
  return `<w:p><w:pPr><w:jc w:val="${align}"/><w:spacing w:line="${line}" w:lineRule="auto" w:after="${after}"/></w:pPr>${runs || '<w:r><w:t></w:t></w:r>'}</w:p>`;
};
const layoutToWord = (bytes, record) => {
  ...
  const paragraphs = (nodes, context) => (nodes || []).map(paragraph => wordParagraph(paragraph, scope, context)).join('');
  const tableXml = table => {
    ...
    const rowXml = ({ row, source, context }) => {
      const cells = (row.cells || []).map((cell, column) => {
        ...
        const content = merge && source !== merge.rowIndex ? '<w:p/>' : paragraphs(cell.content, context);
        const shade = /^#[0-9a-f]{6}$/i.test(cell.background || '') ? `<w:shd w:fill="${cell.background.slice(1)}"/>` : '';
        return `<w:tc><w:tcPr><w:tcW w:w="${width}" w:type="dxa"/>${span > 1 ? `<w:gridSpan w:val="${span}"/>` : ''}${vertical}${shade}<w:vAlign w:val="${cell.verticalAlign === 'middle' ? 'center' : cell.verticalAlign === 'bottom' ? 'bottom' : 'top'}"/></w:tcPr>${content || '<w:p/>'}</w:tc>`;
      }).join('');
  ...
  const blocks = column => (column.blocks || []).map(block => Number(block.type) === 4 ? tableXml(block.table || {}) : paragraphs(block.content, null)).join('');
  ...
  const document = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body>${body}<w:sectPr><w:pgSz w:w="${pageWidth}" w:h="${pageHeight}"${pageWidth > pageHeight ? ' w:orient="landscape"' : ''}/><w:pgMar w:top="${Math.round(Number(setting.paddingTop ?? 5) * 1440 / 25.4)}" w:right="${right}" w:bottom="${Math.round(Number(setting.paddingBottom ?? 5) * 1440 / 25.4)}" w:left="${left}" w:header="0" w:footer="0" w:gutter="0"/></w:sectPr></w:body></w:document>`;
...
const layoutToXlsx = (bytes, record) => {
  const layout = decodeLayout(bytes).content; const scope = templateScope({ fields: layoutFields(layout) }, record); const zip = new PizZip(); const rows = []; const merges = []; const maxColumns = Math.max(1, ...(layout.document.pages || []).flatMap(page => (page.rows || []).flatMap(row => row.columns || []).map(row => row.length || 1)));
  const addRow = (values, mergeAll = false) => { const row = values.map(value => valueText(value)); rows.push(row); if (mergeAll && row.length > 1) merges.push({ row: rows.length, start: 0, end: row.length - 1 }); return row; };
  const cellText = (cell, context) => layoutPlain(cell?.content, scope, context).replace(/\r\n/g, '\n');
  for (const page of layout.document.pages || []) for (const pageRow of page.rows || []) {
    const columns = pageRow.columns || []; for (const column of columns) for (const block of column.blocks || []) {
      if (Number(block.type) !== 4) { for (const paragraph of block.content || []) addRow([layoutPlain(paragraph.children, scope, null)], true); continue; }
      const table = block.table || {}; const dynamic = new Map((table.dynamicRows || []).map(item => [Number(item.rowIndex), cleanLayoutName(item.dataSource?.rootPath?.[0])])); const tableStart = rows.length;
      for (const [sourceIndex, templateRow] of (table.rows || []).entries()) { const root = dynamic.get(sourceIndex); const values = root && matchingValue(scope, root); const contexts = Array.isArray(values) && values.length ? values.map((item, index) => ({ root, item, index })) : [null]; for (const context of contexts) addRow((templateRow.cells || []).map(cell => cellText(cell, context))); }
      for (const merge of table.merges || []) for (let repeat = 0; repeat < Math.max(1, (Array.isArray(matchingValue(scope, dynamic.get(merge.rowIndex))) ? matchingValue(scope, dynamic.get(merge.rowIndex)).length : 1)); repeat++) merges.push({ row: tableStart + merge.rowIndex + 1 + repeat, start: merge.colIndex, end: merge.colIndex + merge.colSpan - 1 });
    }
  }
  const cell = (value, column, row) => `<c r="${excelColumnName(column)}${row}" t="inlineStr"><is><t xml:space="preserve">${xmlEscape(value)}</t></is></c>`;
  const mergeXml = merges.filter(item => item.end > item.start).map(item => `<mergeCell ref="${excelColumnName(item.start)}${item.row}:${excelColumnName(item.end)}${item.row}"/>`).join('');
  const sheetRows = rows.map((row, index) => `<row r="${index + 1}">${row.map((value, column) => cell(value, column, index + 1)).join('')}</row>`).join('');
  ...
  zip.file('xl/worksheets/sheet1.xml', `<?xml version="1.0"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><dimension ref="A1:${excelColumnName(Math.max(0, maxColumns - 1))}${Math.max(1, rows.length)}"/><sheetData>${sheetRows}</sheetData>${mergeXml ? `<mergeCells count="${merges.filter(item => item.end > item.start).length}">${mergeXml}</mergeCells>` : ''}</worksheet>`);
  return zip.generate({ type: 'nodebuffer', compression: 'DEFLATE' });
};
