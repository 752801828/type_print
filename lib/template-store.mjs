...
const htmlToPdf = async (html, outputPath) => {
  const candidates = [process.env.CHROME_PATH, 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe', '/usr/bin/google-chrome-stable', '/usr/bin/google-chrome', '/usr/bin/chromium', '/usr/bin/chromium-browser'].filter(Boolean);
  let chromePath = ''; for (const candidate of candidates) try { await fs.access(candidate); chromePath = candidate; break; } catch {}
  if (!chromePath) throw new Error('服务器未安装 PDF 渲染器，请选择 Word 或 Excel');
