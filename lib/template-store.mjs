...
const layoutFields = layout => {
  ...
  const walk = value => {
    ...
    if (value.type === 'variable' && Array.isArray(value.name)) { const name = String(value.name.at(-1) || '').replace(/^(?:🔵|🟡|🔴|🟢|⚪)\s*/u, '').trim(); if (name && name !== '#' && !name.startsWith('$')) found.push({ marker: name.startsWith('#') ? '#' : '', name: name.replace(/^#/, '') }); }
