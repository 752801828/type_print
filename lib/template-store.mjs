...
const valueText = value => Array.isArray(value) ? value.map(valueText).filter(Boolean).join(value.length && value.every(isTextFragment) ? '' : '、') : value && typeof value === 'object' ? (Object.hasOwn(value, 'text') ? String(value.text ?? '') : Object.hasOwn(value, 'name') ? String(value.name ?? '') : Object.hasOwn(value, 'value') ? valueText(value.value) : JSON.stringify(value)) : String(value ?? '');
