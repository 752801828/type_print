...
const fieldValue = (object, name) => { if (!object || typeof object !== 'object') return undefined; const names = [name, ...(procurementAliases[name] || [])]; const normalized = Object.fromEntries(Object.entries(object).map(([key, value]) => [normalizeKey(key), value])); for (const candidate of names) if (Object.hasOwn(object, candidate)) return object[candidate]; for (const candidate of names) if (normalized[normalizeKey(candidate)] !== undefined) return normalized[normalizeKey(candidate)]; return undefined; };
...
const templateScope = (item, record) => {
  ...
  if (rows.length) { source['__合同明细_采购明细'] = rows; for (const row of rows) { for (const name of ['__开票品名', 'SKU__', '单位', '单价_含税___', '实收数量__', '总价_含税_']) { const value = fieldValue(row, name); if (value !== undefined) row[name] = value; } const quantity = numberValue(fieldValue(row, '实收数量__')); const lineTotal = numberValue(fieldValue(row, '总价_含税_')); if ((row['单价_含税___'] == null || row['单价_含税___'] === '') && quantity) row['单价_含税___'] = lineTotal / quantity; } source['__SKU总数'] = rows.reduce((sum, row) => sum + numberValue(fieldValue(row, '实收数量__')), 0); }
