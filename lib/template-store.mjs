...
export async function readTemplateContent(id) {
  const item = (await readCatalog()).find(entry => entry.id === id);
  if (!item) throw new Error('模板不存在');
  if (!['.txt', '.layout', '.json'].includes(item.extension)) throw new Error('仅支持在线模板编辑');
  return { template: item, content: (await fs.readFile(path.join(templatesDir, item.fileName))).toString('utf8') };
}
