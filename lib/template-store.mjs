...
const layoutPdf = async (bytes, record, outputPath) => {
  const layout = decodeLayout(bytes).content; const scope = templateScope({ fields: layoutFields(layout) }, record); const setting = layout.pageSetting || {}; const font = await pdfFont();
  const firstPage = layout.document.pages?.[0] || {}; const pageWidth = mmToPt(setting.width || 210); const pageHeight = mmToPt(setting.height || 297); const doc = new PDFDocument({ autoFirstPage: false, margin: 0, size: [pageWidth, pageHeight] }); const chunks = [];
  doc.on('data', chunk => chunks.push(chunk)); const done = new Promise((resolve, reject) => { doc.once('end', () => resolve(Buffer.concat(chunks))); doc.once('error', reject); });
  const drawParagraphs = (nodes, x, y, width, rowContext) => { let cursor = y; for (const paragraph of nodes || []) { const fontSize = Math.max(6, parsePt(paragraph.children?.find(child => child.fontSize)?.fontSize || layout.settings?.fontSize || 9)); const value = layoutPlain(paragraph.children, scope, rowContext); if (font) doc.font(font); doc.fontSize(fontSize).fillColor('#111'); const height = Math.max(fontSize * 1.35, doc.heightOfString(value || ' ', { width, align: paragraph.align || 'left' })); doc.text(value, x, cursor, { width, align: paragraph.align || 'left', lineGap: Math.max(0, fontSize * .25) }); cursor += height + mmToPt(Number(paragraph.marginBottom || 0)); } return cursor; };
  const drawTable = (table, x, y, width) => { const columns = table.columns?.length ? table.columns.map(column => width * Number(column.width || 0) / 100) : []; if (!columns.length) return y; const dynamic = new Map((table.dynamicRows || []).map(item => [Number(item.rowIndex), cleanLayoutName(item.dataSource?.rootPath?.[0])])); const merges = table.merges || []; const rows = (table.rows || []).flatMap((row, index) => { const root = dynamic.get(index); const values = root && Array.isArray(matchingValue(scope, root)) ? matchingValue(scope, root) : null; return values?.length ? values.map((item, rowIndex) => ({ row, rowIndex: index, context: { root, item, index: rowIndex } })) : [{ row, rowIndex: index, context: null }]; }); let cursor = y; for (const { row, rowIndex, context } of rows) { const cells = []; let colIndex = 0; for (const cell of row.cells || []) { while (colIndex < columns.length && merges.some(merge => merge.rowIndex === rowIndex && colIndex >= merge.colIndex && colIndex < merge.colIndex + merge.colSpan && merge.colIndex !== colIndex)) colIndex++; const merge = merges.find(item => item.rowIndex === rowIndex && item.colIndex === colIndex); const span = Math.max(1, Number(merge?.colSpan || 1)); const cellWidth = columns.slice(colIndex, colIndex + span).reduce((sum, value) => sum + value, 0); cells.push({ cell, left: x + columns.slice(0, colIndex).reduce((sum, value) => sum + value, 0), width: cellWidth }); colIndex += span; } const heights = cells.map(({ cell, width: cellWidth }) => Math.max(18, doc.heightOfString(layoutPlain((cell.content || []).flatMap(paragraph => paragraph.children || []), scope, context) || ' ', { width: Math.max(12, cellWidth - 8) }) + 8)); const rowHeight = Math.min(260, Math.max(18, ...heights)); for (const { cell, left, width: cellWidth } of cells) { doc.lineWidth(.5).strokeColor('#222').rect(left, cursor, cellWidth, rowHeight).stroke(); drawParagraphs(cell.content || [], left + 4, cursor + 3, Math.max(12, cellWidth - 8), context); } cursor += rowHeight; } return cursor; };
  for (const page of layout.document.pages || []) { doc.addPage({ size: [pageWidth, pageHeight], margin: 0 }); let y = mmToPt(setting.paddingTop || 5); const left = mmToPt(setting.paddingLeft || 13.5); const usable = pageWidth - left - mmToPt(setting.paddingRight || 13.5); for (const row of page.rows || []) { let x = left; const rowStart = y; let rowBottom = y; for (const column of row.columns || []) { const columnWidth = usable * Number(column.width || 100) / 100; let columnY = rowStart; for (const block of column.blocks || []) { columnY = Number(block.type) === 4 ? drawTable(block.table || {}, x, columnY, columnWidth) : drawParagraphs(block.content, x, columnY, columnWidth); } rowBottom = Math.max(rowBottom, columnY); x += columnWidth; } y = rowBottom; } }
