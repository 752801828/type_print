...
const layoutFields = layout => {
  ...
  const walk = value => {
    ...
    if (value.type === 'variable' && Array.isArray(value.name)) { const parts = value.name.map(part => String(part).replace(/^(?:🔵|🟡|🔴|🟢|⚪)\s*/u, '').trim()).filter(Boolean); const name = parts.at(-1) || ''; const path = parts.length > 1 && !['#'].includes(parts[0]) && !parts[0].startsWith('$') ? `${parts[0]}.${name}` : name; if (name && name !== '#' && !name.startsWith('$')) found.push({ marker: name.startsWith('#') ? '#' : '', name: path.replace(/^#/, '') }); }
    for (const dynamic of value.dynamicRows || []) { const name = String(dynamic?.dataSource?.rootPath?.[0] || '').trim(); if (name) found.push({ marker: '#', name }); }
...
const renderTemplateText = (template, scope, loopName = '') => {
  let output = String(template || '').replace(/\[#\]/g, () => scope?.$index === undefined ? '[#]' : String(Number(scope.$index) + 1)); const escapedLoop = loopName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
...
const renderXlsx = (bytes, record, item) => {
  ...
  const renderRow = (rowXml, sourceRow, targetRow, rowScope, loopName = '') => rowXml.replace(/(<row\b[^>]*\br=")\d+("[^>]*>)/, `$1${targetRow}$2`).replace(spreadsheetCellPattern, (cell, attributes, inner) => { const nextAttributes = attributes.replace(/\br="([A-Z]+)\d+"/, `r="$1${targetRow}"`); const source = cellSource(cell); if (!source.includes('{')) { const delta = targetRow - sourceRow; const shifted = delta ? inner.replace(/(<f[^>]*>)([\s\S]*?)(<\/f>)/g, (_, open, formula, close) => `${open}${formula.replace(/(\$?[A-Z]{1,3})(\$?)(\d+)/g, (ref, column, fixed, row) => fixed ? ref : `${column}${Number(row) + delta}`)}${close}`) : inner; return `<c${nextAttributes}>${shifted}</c>`; } const rendered = renderTemplateText(source, rowScope, loopName); const escapedLoop = loopName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); const sourceWithoutLoop = loopName ? source.replace(new RegExp(`\\{#\\s*${escapedLoop}\\s*\\}|\\{/\\s*${escapedLoop}\\s*\\}`, 'gi'), '').trim() : source; const exact = sourceWithoutLoop.match(/^\s*\{\{?\s*([^{}]+?)\s*\}\}?\s*$/); const key = exact?.[1]?.trim() || ''; const scalar = exact && !/^\$index\b/i.test(key) ? numericScalar(resolveTemplateValue(rowScope, key)) : null; if (scalar !== null) { const numericAttributes = nextAttributes.replace(/\s+t="[^"]*"/g, ''); return `<c${numericAttributes}><v>${xmlEscape(scalar)}</v></c>`; } const inlineAttributes = nextAttributes.replace(/\s+t="[^"]*"/g, '') + ' t="inlineStr"'; return `<c${inlineAttributes}><is><t xml:space="preserve">${xmlEscape(rendered)}</t></is></c>`; });
...
// Only same-template packages may share relationships/styles during a body merge.
export function mergeTemplateDocx(buffers) {
  if (buffers.length === 1) return buffers[0];
  const packages = buffers.map(bytes => new PizZip(bytes));
  const first = packages[0];
  const parts = Object.keys(first.files).filter(name => !first.files[name].dir && name !== 'word/document.xml');
  for (const zip of packages.slice(1)) {
    const other = Object.keys(zip.files).filter(name => !zip.files[name].dir && name !== 'word/document.xml');
    if (parts.length !== other.length || parts.some(name => !zip.file(name) || !zip.file(name).asNodeBuffer().equals(first.file(name).asNodeBuffer()))) {
      throw new Error('此模板的页眉/页脚或共享内容随记录变化，暂不支持合并为一个飞书文档；请一次选择一条记录');
    }
  }
  const xml = first.file('word/document.xml').asText();
  const bodies = packages.map(zip => {
    const match = zip.file('word/document.xml').asText().match(/<w:body\b[^>]*>([\s\S]*?)<\/w:body>/);
    if (!match) throw new Error('Word 文档缺少正文');
    return match[1].replace(/<w:sectPr\b[^>]*(?:\/>|>[\s\S]*?<\/w:sectPr>)\s*$/, '');
  });
  const section = xml.match(/<w:sectPr\b[^>]*(?:\/>|>[\s\S]*?<\/w:sectPr>)\s*(?=<\/w:body>)/)?.[0] || '';
  first.file('word/document.xml', xml.replace(/(<w:body\b[^>]*>)[\s\S]*?(<\/w:body>)/, (_, open, close) => `${open}${bodies.join('<w:p><w:r><w:br w:type="page"/></w:r></w:p>')}${section}${close}`));
  return first.generate({ type: 'nodebuffer', compression: 'DEFLATE' });
}

export async function renderFeishuDocx(id, records) {
  const item = (await listTemplates()).find(entry => entry.id === id);
  if (!item) throw new Error('模板不存在');
  if (!Array.isArray(records) || !records.length || records.length > 100 || records.some(record => !record || typeof record !== 'object' || Array.isArray(record))) throw new Error('请选择 1–100 条记录');
  const extension = item.extension || extensionOf(item.fileName);
  if (!['.docx', '.txt', '.layout', '.json'].includes(extension)) throw new Error('第一期飞书文档导出支持 Word 和在线模板；Excel/PDF 请使用原有导出');
  const bytes = await fs.readFile(path.join(templatesDir, item.fileName));
  const rendered = [];
  for (const record of records) rendered.push(extension === '.docx' ? renderDocx(bytes, templateScope(item, record)) : layoutToWord(bytes, record));
  const title = item.outputNamePattern ? renderTemplateText(item.outputNamePattern, templateScope(item, records[0])).trim() : '';
  const name = cleanName(title || path.parse(item.name).name).replace(/\.(?:docx?|xlsx?|pdf|zip)$/i, '') + (records.length > 1 ? `-${records.length}条合并` : '');
  return { bytes: mergeTemplateDocx(rendered), name, template: item };
}
...
export async function readTemplateContent(id) {
  const item = (await readCatalog()).find(entry => entry.id === id);
  if (!item) throw new Error('模板不存在');
  if (!['.txt', '.layout', '.json', '.docx', '.xlsx'].includes(item.extension)) throw new Error('仅支持 TXT、在线模板、DOCX 和 XLSX 编辑');
  const bytes = await fs.readFile(path.join(templatesDir, item.fileName));
  if (item.extension === '.docx') {
    const zip = new PizZip(bytes);
    return { template: item, mode: 'docx-xml', content: zip.file('word/document.xml')?.asText() || '' };
  }
  if (item.extension === '.xlsx') {
    const zip = new PizZip(bytes);
    const sheet = Object.keys(zip.files).find(name => /^xl\/worksheets\/sheet\d+\.xml$/.test(name));
    if (!sheet) throw new Error('XLSX 工作表不存在');
    return { template: item, mode: 'xlsx-xml', sheet, sharedStrings: zip.file('xl/sharedStrings.xml')?.asText() || '', content: zip.file(sheet).asText() };
  }
  return { template: item, mode: 'layout', content: bytes.toString('utf8') };
}
...
export async function updateTemplateContent(id, content) {
  ...
  if (item.extension === '.docx') {
    if (!/<w:document\b[\s\S]*<\/w:document>/.test(value)) throw new Error('DOCX 文档 XML 无效');
    const zip = new PizZip(await fs.readFile(path.join(templatesDir, item.fileName)));
    zip.file('word/document.xml', value);
    const updated = zip.generate({ type: 'nodebuffer' });
    await fs.writeFile(path.join(templatesDir, item.fileName), updated);
    item.fields = extractFields(zip, '.docx');
  } else if (item.extension === '.xlsx') {
    if (!/<worksheet\b[\s\S]*<\/worksheet>/.test(value)) throw new Error('XLSX 工作表 XML 无效');
    const zip = new PizZip(await fs.readFile(path.join(templatesDir, item.fileName)));
    const sheet = Object.keys(zip.files).find(name => /^xl\/worksheets\/sheet\d+\.xml$/.test(name));
    if (!sheet) throw new Error('XLSX 工作表不存在');
    zip.file(sheet, value);
    await fs.writeFile(path.join(templatesDir, item.fileName), zip.generate({ type: 'nodebuffer' }));
    item.fields = extractFields(zip, '.xlsx');
  } else {
    const layout = decodeLayout(Buffer.from(value, 'utf8')).content;
    if (!layout?.document?.pages?.length) throw new Error('在线模板内容无效');
    await fs.writeFile(path.join(templatesDir, item.fileName), value, 'utf8');
    item.fields = layoutFields(layout);
    item.stats = { pages: layout.document.pages.length, blocks: JSON.stringify(layout).match(/"type":/g)?.length || 0 };
  }
  await writeCatalog(await readCatalog().then(catalog => catalog.map(entry => entry.id === id ? item : entry)));
  return item;
}
