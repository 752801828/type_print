...
const layoutToXlsx = (bytes, record) => {
  ...
  const renderTable = (table, startRow, startColumn, span, regionWidth) => {
    ...
    for (let index = 0; index < Math.min(span, tableColumns); index++) columnWidths[startColumn + index] = regionWidth * weights[index] / totalWeight;
    const heightNeeds = [];
    for (const [targetOffset, entry] of entries.entries()) {
      ...
      for (const [column, cell] of (entry.templateRow.cells || []).entries()) {
        ...
        addCell(rowIndex, startColumn + column, value, paragraphStyle(paragraphs[0], cell, true));
