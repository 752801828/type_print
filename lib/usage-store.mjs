...
const resolveUserName = async userId => { if (!userId || !process.env.FEIYE_FEISHU_APP_ID || !process.env.FEIYE_FEISHU_APP_SECRET) return ''; const userIdType = userId.startsWith('ou_') ? 'open_id' : userId.startsWith('on_') ? 'union_id' : 'user_id'; try { const token = await tenantAccessToken(); if (!token) return ''; const result = await requestJson('GET', `https://open.feishu.cn/open-apis/contact/v3/users/${encodeURIComponent(userId)}?user_id_type=${userIdType}`, null, { authorization: `Bearer ${token}` }); return clean(result.data?.user?.name || result.data?.user?.en_name || result.data?.user?.enName); } catch { return ''; } };

export async function recordUsage(value = {}) {
  const userId = clean(value.userId, 200);
  const userName = clean(value.userName) || await resolveUserName(userId);
  const event = {
