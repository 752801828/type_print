...
const editableLayout = layout => {
  ...
  const walk = value => { if (!value || typeof value !== 'object') return; if (value.type === 'variable') return; if (value.type && (value.text !== undefined || value.children) && !value.blocks && !value.content && !value.columns && !value.rows && !value.table) { nodes.push(value); return; } for (const child of Object.values(value)) walk(child); };
  walk(layout);
  const elements = [...root.querySelectorAll('span:not(.layout-variable):not(.layout-value)')]; editorTextBindings = elements.slice(0, nodes.length).map((element, index) => { element.contentEditable = 'true'; element.classList.add('template-editor-text'); element.spellcheck = false; return { element, node: nodes[index] }; });
  return root;
};
