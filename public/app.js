...
const state = { fields: [], viewFields: [], records: [], templates: [], selectedTemplate: null, context: null, table: null, view: null, userId: '' };
...
async function initUserMemory() { if (state.userId) return; try { state.userId = String(await sdk?.bitable?.bridge?.getUserId?.() || ''); } catch {} userMemory = readUserMemory(); }
