...
const recordScope = record => { const values = {}; for (const field of state.fields) assignFieldAliases(values, field.name, text(record.fields[field.id])); for (const templateField of state.selectedTemplate?.fields || []) if (templateField.marker !== '#') { const sourceField = findTemplateField(state.fields, templateField.name); if (sourceField) values[templateField.name] = text(record.fields[sourceField.id]); } values['表格名'] = state.context?.tableName || ''; values['打印时间'] = new Date().toLocaleString('zh-CN'); for (const [name, rows] of Object.entries(record.loops || {})) assignFieldAliases(values, name, rows); applyProcurementRules(values, record); for (const field of state.selectedTemplate?.fields || []) if (matchingValue(values, field.name) === undefined) values[field.name] = field.marker === '#' ? [] : ''; return values; };
...
const drawGenerateMenu = () => { const item = state.selectedTemplate; const attachments = state.fields.filter(field => Number(field.type) === 17); const valid = attachments.some(field => field.id === item?.outputFieldId); $('outputFieldId').innerHTML = attachments.length ? attachments.map(field => `<option value="${escapeHtml(field.id)}">${escapeHtml(field.name)}</option>`).join('') : '<option value="">当前表没有附件字段</option>'; $('outputFieldId').value = valid ? item.outputFieldId : attachments[0]?.id || ''; $('autoUpload').checked = Boolean(item?.autoUpload && valid); $('autoUpload').disabled = !attachments.length; $('outputReplace').checked = Boolean(item?.outputReplace); updateAutoUploadControls(); };
...
async function saveGenerateOptions() { if (!state.selectedTemplate) return; if ($('autoUpload').checked && !$('outputFieldId').value) return toast('当前表没有可用的附件字段', 'error'); try { const response = await fetch(appUrl(`/api/templates/${state.selectedTemplate.id}`), { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ autoUpload: $('autoUpload').checked, outputFieldId: $('outputFieldId').value, outputReplace: $('outputReplace').checked }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error || '保存失败'); Object.assign(state.selectedTemplate, result.template); } catch (error) { drawGenerateMenu(); toast(error.message, 'error'); } }
async function openTemplateSettings(id = state.selectedTemplate?.id) { const item = state.templates.find(template => template.id === id); if (!item) return toast('请先选择模板', 'error'); state.selectedTemplate = item; hideMoreMenu(); $('templateName').value = item.name || ''; $('outputNamePattern').value = item.outputNamePattern || ''; $('nameFieldList').hidden = true; $('settingsDialog').showModal(); await ensureLinkedSchemas(); if ($('outputNamePattern').value) drawNameFieldSuggestions(); }
...
$('autoUpload').onchange = () => { updateAutoUploadControls(); saveGenerateOptions(); };
$('outputFieldId').onchange = saveGenerateOptions;
$('outputReplace').onchange = saveGenerateOptions;
$('generateMenu').onclick = event => event.stopPropagation();
$('saveSettings').onclick = async () => { if (!state.selectedTemplate) return; try { const response = await fetch(appUrl(`/api/templates/${state.selectedTemplate.id}`), { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name: $('templateName').value, outputNamePattern: $('outputNamePattern').value }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error || '保存失败'); Object.assign(state.selectedTemplate, result.template); drawTemplates(); closeTemplateSettings(); await readCurrentRecord(true); toast('排版设置已保存', 'success'); } catch (error) { toast(error.message, 'error'); } };
...
async function uploadGeneratedFiles(output) {
  if (!state.selectedTemplate?.autoUpload) return 0;
  const field = state.fields.find(item => item.id === state.selectedTemplate.outputFieldId && Number(item.type) === 17); if (!field) throw new Error('目标附件字段不存在或不可写');
  ...
  await Promise.all(downloaded.map(async ({ item, file }, index) => { const record = state.records[item.recordIndex]; let existing = record.fields[field.id]; if (!state.selectedTemplate.outputReplace && existing === undefined) try { existing = await state.table.getCellValue(field.id, record.id); } catch {} const attachments = attachmentCellValue(existing, file, tokens[index], state.selectedTemplate.outputReplace); await state.table.setCellValue(field.id, record.id, attachments); record.fields[field.id] = attachments; }));
...
$('generate').onclick = async () => {
  ...
  try {
    const records = state.records.map(recordScope); const response = await fetch(appUrl('/api/generate-docx'), { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ templateId: state.selectedTemplate.id, records, outputFormat: $('outputFormat').value, keepFiles: Boolean(state.selectedTemplate.autoUpload) }), signal: controller.signal }); const result = await response.json(); if (!response.ok || !result.output) throw new Error(result.error || '生成失败');
    clearInterval(progressTimer); let uploaded = 0; let uploadError = ''; if (state.selectedTemplate.autoUpload) { $('generationPercent').textContent = '正在上传'; $('result').querySelector('small').textContent = '正在把文件写入对应记录…'; try { uploaded = await uploadGeneratedFiles(result.output); } catch (error) { uploadError = error.message || '上传失败'; } }
