...
function drawNameFieldSuggestions() { const input = $('outputNamePattern'); const fragment = filenameFieldFragment(input.value, input.selectionStart); const list = $('nameFieldList'); const matches = matchingFilenameFields(state.fields || [], fragment.query); list.innerHTML = matches.map(name => `<button type="button" role="option" data-name-field="${escapeHtml(name)}"><span>{${escapeHtml(name)}}</span><small>插入字段</small></button>`).join(''); list.hidden = !matches.length; input.setAttribute('aria-expanded', String(Boolean(matches.length))); list.querySelectorAll('[data-name-field]').forEach(button => { button.onmousedown = event => event.preventDefault(); button.onclick = event => { event.preventDefault(); event.stopPropagation(); const current = filenameFieldFragment(input.value, input.selectionStart); input.setRangeText(`{${button.dataset.nameField}}`, current.start, current.end, 'end'); list.hidden = true; input.setAttribute('aria-expanded', 'false'); input.focus({ preventScroll: true }); }; }); }
const closeTemplateSettings = () => { $('settingsPanel').classList.remove('open'); $('settingsPanel').setAttribute('aria-hidden', 'true'); $('settingsPanel').inert = true; $('nameFieldList').hidden = true; };
...
function openTemplateSettings(id) { const item = state.templates.find(template => template.id === id); if (!item) return; state.selectedTemplate = item; setMenuOpen(true); $('outputNamePattern').value = item.outputNamePattern || ''; $('nameFieldList').hidden = true; $('settingsPanel').inert = false; $('settingsPanel').classList.add('open'); $('settingsPanel').setAttribute('aria-hidden', 'false'); }
...
$('saveSettings').onclick = async () => { if (!state.selectedTemplate) return; try { const response = await fetch(appUrl(`/api/templates/${state.selectedTemplate.id}`), { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ outputNamePattern: $('outputNamePattern').value }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error || '保存失败'); Object.assign(state.selectedTemplate, result.template); drawTemplates(); closeTemplateSettings(); await readCurrentRecord(true); toast('模板设置已保存', 'success'); } catch (error) { toast(error.message, 'error'); } };
document.addEventListener('click', event => { hideTemplateContextMenu(); if ($('settingsPanel').classList.contains('open') && !$('settingsPanel').contains(event.target) && !event.target.closest('[data-settings-id]')) closeTemplateSettings(); });
