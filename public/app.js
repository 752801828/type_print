...
const updateOutputOptions = item => { const current = $('outputFormat').value; const extension = String(item?.extension || '.docx').slice(1).toLowerCase(); const options = isLayoutTemplate(item) ? [['word','Word'],['xlsx','Excel（XLSX）'],['pdf','PDF']] : [[extension, outputLabel(extension)]]; $('outputFormat').innerHTML = options.map(([value, label]) => `<option value="${value}">${label}</option>`).join(''); $('outputFormat').value = options.some(([value]) => value === current ? value !== 'docx' : false) ? current : options[0][0]; };
...
async function readField(field, recordId, record, table) { if (storedCell(field, record)) { const value = record.fields[field.id] ?? ''; return isDisplayField(field) ? displayFieldValue(field, recordId, value) : value; } let value; try { value = await table?.getCellValue?.(field.id, recordId); } catch {} if (isDisplayField(field) && hasCellValue(value)) return displayFieldValue(field, recordId, value); if (!hasCellValue(value)) { try { value = await field.api.getValue(record || recordId); } catch {} } if (!hasCellValue(value)) { try { value = await field.api.getValue(recordId); } catch {} } if (isDisplayField(field)) return displayFieldValue(field, recordId, value); if (!hasCellValue(value)) { try { value = await field.api.getCellString(recordId); } catch {} } if (!hasCellValue(value)) value = fallbackRecordField(field, record); return value ?? ''; }
...
$('generate').onclick = async () => {
  ...
  const missing = templateFieldDiagnostics().filter(item => !item.matched); if (missing.length) return toast(`模板有 ${missing.length} 个字段未匹配：${missing.map(item => item.name).join('、')}，请先修改模板`, 'error');
  const button = $('generate'); const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), 45000); let progress = 8; button.disabled = true; button.textContent = '生成中…'; $('result').hidden = false; $('result').className = 'result generating'; $('result').innerHTML = '<div class="generation-status"><strong>正在生成文件</strong><span id="generationPercent">8%</span></div><div class="progress-track" role="progressbar" aria-label="文件生成进度" aria-valuemin="0" aria-valuemax="100" aria-valuenow="8"><i id="generationProgress" style="width:8%"></i></div><small>正在整理记录并写入模板…</small>'; const progressTimer = setInterval(() => { progress = Math.min(90, progress + Math.max(1, Math.round((92 - progress) / 7))); $('generationProgress').style.width = `${progress}%`; $('generationProgress').parentElement.setAttribute('aria-valuenow', progress); $('generationPercent').textContent = `${progress}%`; }, 350);
  try {
