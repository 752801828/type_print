...
async function initUserMemory() { if (state.userId) return; try { state.userId = String(await sdk?.bitable?.bridge?.getUserId?.() || ''); } catch {} try { state.userName = localStorage.getItem(userNameKey()) || ''; } catch {} userMemory = readUserMemory(); }
const rememberCurrentUserName = fields => { if (!state.userId || state.userName) return; for (const field of state.fields.filter(item => [11, 1003, 1004].includes(Number(item.type)))) { const users = Array.isArray(fields?.[field.id]) ? fields[field.id] : [fields?.[field.id]]; const user = users.find(item => item && typeof item === 'object' && String(item.id || item.userId || item.openId || item.user_id || item.open_id || '') === state.userId && (item.name || item.enName || item.en_name)); if (!user) continue; state.userName = cleanInvisible(user.name || user.enName || user.en_name).trim(); try { localStorage.setItem(userNameKey(), state.userName); } catch {} break; } };
