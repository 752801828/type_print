...
const bindSelectionListener = () => { if (selectionBound || !sdk?.bitable?.base) return; selectionBound = true; try { sdk.bitable.base.onSelectionChange(() => { clearTimeout(selectionTimer); selectionTimer = setTimeout(readCurrentRecord, 80); }); } catch {} selectionPoll = setInterval(async () => { try { const current = await sdk.bitable.base.getSelection(); const checked = state.view?.getSelectedRecordIdList ? await state.view.getSelectedRecordIdList() : []; const ids = currentReadRecordIds(current, checked); const key = `${current.baseId || ''}/${current.tableId || ''}/${current.viewId || ''}/${ids.join(',')}`; if (state.context?.selectionKey && state.context.selectionKey !== key) readCurrentRecord(); } catch {} }, 700); };
readCurrentRecord();
