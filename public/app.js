...
const state = { fields: [], viewFields: [], records: [], templates: [], selectedTemplate: null, context: null, table: null, view: null, userId: '', userName: '' };
...
async function initUserMemory() { if (state.userId) return; const bridge = sdk?.bitable?.bridge; try { state.userId = String(await sdk?.bitable?.bridge?.getUserId?.() || ''); } catch {} try { state.userName = localStorage.getItem(userNameKey()) || ''; } catch {} if (!state.userName) for (const method of ['getUserInfo', 'getCurrentUser', 'getUser']) { try { const name = profileName(await bridge?.[method]?.()); if (name) { state.userName = name; try { localStorage.setItem(userNameKey(), name); } catch {} break; } } catch {} } userMemory = readUserMemory(); }
...
const rememberCurrentUserName = fields => { if (!state.userId || state.userName) return; const wanted = state.userId.trim().toLowerCase(); for (const field of state.fields.filter(item => [11, 1003, 1004].includes(Number(item.type)))) { const user = userValueItems(fields?.[field.id]).find(item => userIdentity(item).toLowerCase() === wanted && (item.name || item.enName || item.en_name || item.displayName || item.display_name)); if (!user) continue; state.userName = cleanInvisible(user.name || user.enName || user.en_name || user.displayName || user.display_name).trim(); try { localStorage.setItem(userNameKey(), state.userName); } catch {} break; } };
