...
function drawSourceVariables() { const query = normalizeKey($('sourceVariableSearch').value); const fields = variableFields().filter(item => !query || normalizeKey(`${item.name}${item.tableName || ''}`).includes(query)); const groups = new Map(); for (const item of fields) { const group = item.group || '本表字段'; if (!groups.has(group)) groups.set(group, []); groups.get(group).push(item); } $('sourceVariableList').innerHTML = [...groups].map(([group, items]) => `<section class="source-variable-group"><h3>${escapeHtml(group)}<small>${group === '本表字段' ? `${items.length} 个字段` : `关联表 · ${items.length} 个字段`}</small></h3>${items.map(item => `<button type="button" data-copy-variable="${escapeHtml(item.label)}"><span class="source-field-icon">${fieldIcon(item.field)}</span><span>${escapeHtml(item.label)}</span><b title="复制占位符">▣</b></button>`).join('')}</section>`).join('') || '<div class="source-empty">没有匹配的变量</div>'; $('sourceVariableList').querySelectorAll('[data-copy-variable]').forEach(button => button.onclick = () => copyVariable(button.dataset.copyVariable)); }
