...
async function openTemplateEditor(id = state.selectedTemplate?.id) { const item = state.templates.find(template => template.id === id); if (!item) return toast('请先选择模板', 'error'); if (!isEditableTemplate(item)) return toast('仅支持 TXT、在线模板、DOCX 和 XLSX 编辑', 'error'); try { const response = await fetch(appUrl(`/api/templates/${id}/content`)); const data = await response.json(); if (!response.ok) throw new Error(data.error || '读取模板失败'); $('templateEditorContent').value = data.content || ''; $('templateEditorDialog').dataset.templateId = id; $('templateEditorDialog').dataset.mode = data.mode || ''; $('templateEditorDialog').querySelector('.settings-subtitle').textContent = data.mode === 'docx-xml' ? 'DOCX 高级编辑：修改正文 XML 可保留原有排版结构。' : data.mode === 'xlsx-xml' ? 'XLSX 高级编辑：修改第一个工作表 XML 可保留原有样式结构。' : '编辑在线模板内容，保存前会自动校验布局格式。'; $('templateEditorDialog').showModal(); $('templateEditorContent').focus(); } catch (error) { toast(error.message, 'error'); } }
