...
test('renames and duplicates a bound template with its settings', async () => {
  ...
  try {
    const renamed = await updateTemplate(original.id, { name: '新名称.pdf' }); assert.equal(renamed.name, '新名称.docx'); assert.equal(renamed.outputNamePattern, '{客户}-合同'); assert.equal(renamed.outputFieldId, 'attachment');
    copy = await duplicateTemplate(original.id); assert.equal(copy.name, '新名称-副本.docx'); assert.equal(copy.tableId, 'table'); assert.equal(copy.outputNamePattern, '{客户}-合同'); assert.equal(copy.autoUpload, true); assert.equal(copy.outputFieldId, 'attachment'); assert.equal(copy.outputReplace, true); assert.deepEqual(await fs.readFile(templateFile(copy.id, '.docx')), await fs.readFile(templateFile(original.id, '.docx')));
