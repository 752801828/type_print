...
$('generate').onclick = async () => {
  ...
  const button = $('generate'); const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), 45000); button.disabled = true; button.textContent = '生成中…';
  try {
    ...
    const downloadUrl = appUrl(`/api/outputs/${result.output.id}/download?name=${encodeURIComponent(result.output.name)}`); $('result').hidden = false; $('result').innerHTML = `生成完成。<a href="${downloadUrl}" download="${escapeHtml(result.output.name)}">下载 ${escapeHtml(result.output.name)} ↗</a>`; toast(`${records.length} 条记录已生成`, 'success');
