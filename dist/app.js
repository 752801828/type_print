...
const closeTemplateSettings = () => { $('settingsPanel').classList.remove('open'); $('settingsPanel').setAttribute('aria-hidden', 'true'); $('settingsPanel').inert = true; };
...
function openTemplateSettings(id) { const item = state.templates.find(template => template.id === id); if (!item) return; state.selectedTemplate = item; setMenuOpen(true); $('outputNamePattern').value = item.outputNamePattern || ''; $('nameFieldList').innerHTML = (state.fields || []).map(field => `<button data-name-field="${escapeHtml(field.name)}">{${escapeHtml(field.name)}}</button>`).join('') || '<span>当前表格没有可用字段</span>'; $('nameFieldList').querySelectorAll('[data-name-field]').forEach(button => button.onclick = () => { const input = $('outputNamePattern'); const tag = `{${button.dataset.nameField}}`; input.setRangeText(tag, input.selectionStart, input.selectionEnd, 'end'); input.focus(); }); $('settingsPanel').inert = false; $('settingsPanel').classList.add('open'); $('settingsPanel').setAttribute('aria-hidden', 'false'); }
