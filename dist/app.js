...
async function openTemplateEditor(id = state.selectedTemplate?.id) { const item = state.templates.find(template => template.id === id); if (!item) return toast('请先选择模板', 'error'); if (!isEditableTemplate(item)) return toast('仅支持 TXT、在线模板、DOCX 和 XLSX 编辑', 'error'); try { const response = await fetch(appUrl(`/api/templates/${id}/content`)); const data = await response.json(); if (!response.ok) throw new Error(data.error || '读取模板失败'); const canvas = $('templateEditorCanvas'); canvas.replaceChildren(); editorDocument = null; editorLayout = null; editorTextBindings = []; $('templateEditorDialog').dataset.templateId = id; $('templateEditorDialog').dataset.mode = data.mode || ''; if (data.mode === 'layout') { editorDocument = JSON.parse(data.content); editorLayout = typeof editorDocument.content === 'string' ? JSON.parse(editorDocument.content) : (editorDocument.content || editorDocument); canvas.append(editableLayout(editorLayout)); $('templateEditorDialog').querySelector('.settings-subtitle').textContent = '直接点击页面文字编辑，变量占位符保持不变。'; $('saveTemplateEditor').disabled = false; } else { canvas.innerHTML = `<div class="template-editor-notice">${data.mode === 'docx-xml' ? 'DOCX 暂不支持浏览器内可视化排版编辑。' : 'XLSX 暂不支持浏览器内可视化表格编辑。'}<br>请使用“导出”后在 Word/Excel 中编辑，再重新上传模板。</div>`; $('templateEditorDialog').querySelector('.settings-subtitle').textContent = '当前文件格式需要原生 Office 编辑器才能完整保留排版。'; $('saveTemplateEditor').disabled = true; } $('templateEditorContent').value = data.content || ''; $('templateEditorDialog').showModal(); if (data.mode === 'layout') editorTextBindings[0]?.element.focus(); } catch (error) { toast(error.message, 'error'); } }
...
$('saveTemplateEditor').onclick = async () => { const dialog = $('templateEditorDialog'); const id = dialog.dataset.templateId; if (!id || dialog.dataset.mode !== 'layout') return; const button = $('saveTemplateEditor'); button.disabled = true; try { for (const { element, node } of editorTextBindings) node.text = element.textContent; const documentValue = { ...editorDocument }; if (typeof editorDocument.content === 'string') documentValue.content = JSON.stringify(editorLayout); else if (editorDocument.content) documentValue.content = editorLayout; const response = await fetch(appUrl(`/api/templates/${id}/content`), { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ content: JSON.stringify(documentValue) }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error || '保存模板失败'); const item = state.templates.find(template => template.id === id); if (item) Object.assign(item, result.template); closeTemplateEditor(); drawTemplates(); toast('在线模板已保存', 'success'); } catch (error) { toast(error.message, 'error'); } finally { button.disabled = false; } };
