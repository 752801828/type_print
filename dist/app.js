...
const text = value => Array.isArray(value) ? value.map(text).filter(Boolean).join(value.length && value.every(isTextFragment) ? '' : '、') : value && typeof value === 'object' ? (Object.hasOwn(value, 'text') ? String(value.text ?? '') : Object.hasOwn(value, 'name') ? String(value.name ?? '') : Object.hasOwn(value, 'value') ? text(value.value) : JSON.stringify(value)) : value == null ? '' : String(value);
...
async function displayDateField(field, recordId, value) { try { const display = await field.api.getCellString(recordId); if (hasCellValue(display)) return display; } catch {} return timestampText(field, value); }
async function readField(field, recordId, record, table) { if (storedCell(field, record)) { const value = record.fields[field.id] ?? ''; return isDateField(field) && hasCellValue(value) ? displayDateField(field, recordId, value) : value; } let value; try { value = await table?.getCellValue?.(field.id, recordId); } catch {} if (isDateField(field) && hasCellValue(value)) return displayDateField(field, recordId, value); if (!hasCellValue(value)) { try { value = await field.api.getValue(record || recordId); } catch {} } if (!hasCellValue(value)) { try { value = await field.api.getValue(recordId); } catch {} } if (!hasCellValue(value)) { try { value = await field.api.getCellString(recordId); } catch {} } if (!hasCellValue(value)) value = fallbackRecordField(field, record); return value ?? ''; }
...
function drawSourceVariables() { const query = normalizeKey($('sourceVariableSearch').value); const fields = variableFields().filter(item => !query || normalizeKey(`${item.name}${item.tableName || ''}`).includes(query)); const groups = new Map(); for (const item of fields) { const group = item.group || '本表字段'; if (!groups.has(group)) groups.set(group, []); groups.get(group).push(item); } $('sourceVariableList').innerHTML = [...groups].map(([group, items]) => `<section class="source-variable-group"><h3>${escapeHtml(group)}<small>${group === '本表字段' ? `${items.length} 个字段` : `关联表 · ${items.length} 个字段`}</small></h3>${items.map(item => `<button type="button" data-copy-variable="${escapeHtml(item.label)}"><span class="source-field-icon">${fieldIcon(item.field)}</span><span>${escapeHtml(item.label)}</span><span class="source-copy-button">复制</span></button>`).join('')}</section>`).join('') || '<div class="source-empty">没有匹配的变量</div>'; $('sourceVariableList').querySelectorAll('[data-copy-variable]').forEach(button => button.onclick = () => copyVariable(button.dataset.copyVariable)); }
