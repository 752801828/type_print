...
async function readLinkedRows(raw, bitable, fallbackTableId = '') {
  ...
  try {
    ...
    return Promise.all(ids.map(async id => {
      ...
      for (const field of linkedFields) { if (isDateField(field) || ![2, 19, 20].includes(Number(field.type))) continue; const raw = rawRecord?.fields?.[field.id]; const candidate = raw && typeof raw === 'object' && raw.value !== undefined ? raw.value : raw; const display = row[field.name]; if (Number.isFinite(Number(candidate)) && /^-?(?:\d+\.?\d*|\.\d+)$/.test(String(display || '').trim())) numericFields[field.name] = Number(candidate); }
