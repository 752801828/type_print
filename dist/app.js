...
$('generate').onclick = async () => {
  ...
  try {
    const records = state.records.map(recordScope); const response = await fetch(appUrl('/api/generate-docx'), { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ templateId: state.selectedTemplate.id, records, outputFormat: $('outputFormat').value }), signal: controller.signal }); const result = await response.json(); if (!response.ok || !result.output) throw new Error(result.error || '生成失败');
    clearInterval(progressTimer); const downloadUrl = appUrl(`/api/outputs/${result.output.id}/download?name=${encodeURIComponent(result.output.name)}`); $('result').className = 'result generated'; $('result').innerHTML = `<div class="generation-status"><strong>生成完成</strong><span>100%</span></div><div class="progress-track done"><i style="width:100%"></i></div><div class="download-row"><span>${escapeHtml(result.output.name)}</span><a id="automaticDownload" href="${downloadUrl}" download="${escapeHtml(result.output.name)}">重新下载</a></div>`; $('automaticDownload').click(); toast(`${records.length} 条记录已生成并开始下载`, 'success');
