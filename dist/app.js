...
async function openTemplateSettings(id = state.selectedTemplate?.id) { const item = state.templates.find(template => template.id === id); if (!item) return toast('请先选择模板', 'error'); state.selectedTemplate = item; hideMoreMenu(); $('templateName').value = item.name || ''; $('outputNamePattern').value = item.outputNamePattern || ''; const attachments = state.fields.filter(field => Number(field.type) === 17); $('outputFieldId').innerHTML = attachments.length ? attachments.map(field => `<option value="${escapeHtml(field.id)}">${escapeHtml(field.name)}</option>`).join('') : '<option value="">当前表没有附件字段</option>'; $('outputFieldId').value = attachments.some(field => field.id === item.outputFieldId) ? item.outputFieldId : attachments[0]?.id || ''; $('autoUpload').checked = Boolean(item.autoUpload && $('outputFieldId').value); $('autoUpload').disabled = !attachments.length; $('outputReplace').checked = Boolean(item.outputReplace); updateAutoUploadControls(); $('nameFieldList').hidden = true; $('settingsDialog').showModal(); await ensureLinkedSchemas(); if ($('outputNamePattern').value) drawNameFieldSuggestions(); }
...
document.addEventListener('click', () => { hideTemplateContextMenu(); hideMoreMenu(); });
...
async function uploadGeneratedFiles(output) {
  ...
  await Promise.all(downloaded.map(async ({ item, file }, index) => { const record = state.records[item.recordIndex]; const attachments = attachmentCellValue(record.fields[field.id], file, tokens[index], state.selectedTemplate.outputReplace); await state.table.setCellValue(field.id, record.id, attachments); record.fields[field.id] = attachments; }));
