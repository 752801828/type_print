...
const activeRecordFields = () => {
  ...
  for (const match of String(state.selectedTemplate?.outputNamePattern || '').matchAll(/\{([^{}]+)\}/g)) add(findTemplateField(state.fields, match[1].trim()));
...
$('addNameVariable').onclick = event => { event.stopPropagation(); if ($('nameFieldList').hidden) drawNameFieldSuggestions(true); else { $('nameFieldList').hidden = true; $('outputNamePattern').setAttribute('aria-expanded', 'false'); } };
